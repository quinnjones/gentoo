...
function cleanup
{
    ...
    if !( mount | grep '[[:space:]]on /var/db/repos/gentoo[[:space:]]' ); then
        mount "$MNT"
    fi
}
...
_debug "SRC=$SRC DST=$DST MNT=$MNT"
...
_verbose "Examining new squashed portage"
_debug "cp '$SRC' '$TMP'"
cp "$SRC" "$TMP"

_verbose "checking whether portage was updated"
oldmd5=$(md5sum $DST | awk '{print $1}')
newmd5=$(md5sum $TMP | awk '{print $1}')
_debug "oldmd5:$oldmd5 newmd5:$newmd5"

if [[ "$oldmd5" == "$newmd5" ]]; then
    _verbose "No updates found"
else
    _verbose "portage has updates, applying them"
    ...
    _debug "mount -t squashfs '$DST' '$MNT'"
    mount -t squashfs "$DST" "$MNT"

    _verbose "Updating eix cache"
    _debug "eix-update"
    eix-update
fi
